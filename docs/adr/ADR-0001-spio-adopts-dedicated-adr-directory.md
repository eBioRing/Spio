# ADR-0001: Spio Adopts a Dedicated ADR Directory

**Purpose:** Record the decision, context, alternatives, and consequences for storing durable `spio` design and implementation decisions under `docs/adr/`.
...
`spio` began as a bootstrap-heavy repository with decisions spread across planning documents, governance contracts, and implementation patches. That was workable for early scaffolding, but it made it too easy for design rationale and accepted implementation boundaries to disappear into code or chat history.
...
1. Add a dedicated `docs/adr/` directory to the `spio` repository.
...
1. `spio` now has a stable place for durable decision records.

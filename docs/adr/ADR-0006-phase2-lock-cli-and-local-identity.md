# ADR-0006: Phase-2 `spio lock` CLI Surface and Local Identity Rules

**Purpose:** Record the decision, context, alternatives, and consequences for the first public `spio lock` command shape, stale-check behavior, and local lock package identity rules.
...
`ADR-0005` freezes the phase-2 scope of `spio lock` to local package, workspace, and recursive `path` graphs, with explicit rejection of `git` resolution. That leaves three public-contract questions that cannot stay implicit in code:

1. what argument surface phase-2 `spio lock` exposes
...
1. Freeze the phase-2 public `spio lock` surface as:
   - `spio lock`
   - `spio lock --manifest-path <path>`
   - `spio lock --check`
   - `spio lock --manifest-path <path> --check`
2. The lockfile path is not a free argument in phase 2.
   - The command always reads or writes the adjacent `spio.lock` next to the selected manifest.
3. `--check` compares the generated canonical lockfile to the on-disk adjacent `spio.lock`.
...
7. `git` dependencies remain valid manifest input in phase 2 but `spio lock` must fail explicitly when it reaches them.
...
1. `spio lock` has a small, explicit, stable phase-2 surface.

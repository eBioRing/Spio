# ADR-0008: Hermetic Git Snapshot Cache Lives Under `SPIO_HOME`
...
Phase-3 pinned `git` resolution needs a reproducible place to store fetched source state. The project already committed to hermetic `SPIO_HOME` usage and to avoiding dependence on developer-local mutable state, but it had not yet frozen the first concrete cache layout.
...
1. The first phase-3 git source cache lives under `SPIO_HOME`.
2. `SPIO_HOME` resolves as:
   - the `SPIO_HOME` environment variable when set
   - otherwise `~/.spio`
3. The minimal git cache layout is:
   - `SPIO_HOME/git/repos/<repo-hash>.git` for the mirrored repository cache
   - `SPIO_HOME/git/checkouts/<repo-hash>/<rev>/` for the extracted revision snapshot
...
7. Tests and gates must continue to use a fresh temporary `SPIO_HOME`.

## Alternatives

1. Fetch git sources directly into the project-local `.spio/`.
...
3. Test and gate isolation rules remain compatible with the existing `SPIO_HOME` discipline.

# ADR-0014: Phase-4 Activates `pafio test --dry-run` with Explicit `[[test]]` Targets
...
Earlier phase-4 work intentionally left `pafio test` stubbed because the project did not yet freeze where test targets came from. The current compile-plan schema already allows `intent = "test"` and `entry.target_kind = "test"`, but package target metadata still needed a stable source.
...
1. Freeze explicit package tests as `[[test]]` entries in `pafio.toml`.
...
3. Activate `pafio test` in phase 4 with the minimal public surface:
   - `pafio test [--manifest-path <path>] [--package <package-name>] [--test <name>] [--profile <dev|release>] [--dry-run] [--styio-bin <path>]`
4. `pafio test --dry-run` uses the same compile-plan pipeline as `build` and `run`, but emits `intent = "test"`.
5. `pafio test` only targets explicit manifest `[[test]]` entries.
...
6. Non-dry-run `pafio test` follows the same published compile-plan compatibility gate as `build` and `run`.
   - under the current compatibility matrix it fails before compiler execution begins

## Alternatives

1. Continue leaving `pafio test` stubbed until the compiler consumer ships.
...
3. `pafio test --dry-run` can validate entry selection and package target metadata before compiler integration is published.

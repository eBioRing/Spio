# ADR-0017: Phase-6 Activates Managed Local `styio` Installation Through `spio tool install`

**Purpose:** Record the decision, context, alternatives, and consequences for the first native `tool install` surface and the managed local compiler layout under `SPIO_HOME`.
...
`spio` now has native resolver, workflow-planning, source-package, and publish-preflight paths, but the last reserved public command is still `spio tool install`. Leaving it stubbed would keep the package-manager surface incomplete.
...
`spio` already depends on a published `styio --machine-info=json` handshake and a package-manager-side compatibility matrix. That makes local compiler admission and managed reuse possible without inventing any new remote protocol.
...
4. `tool install` must validate the selected compiler through the existing public compatibility path before writing managed state.
   ...
   - it reuses `spio/contracts/compat/styio-support.toml`
5. Managed install layout lives under hermetic `SPIO_HOME/tools/styio/`.
   - versioned install root: `SPIO_HOME/tools/styio/<channel>/<compiler-version>/`
   - managed default root: `SPIO_HOME/tools/styio/current/`
...
7. Compiler selection order becomes:
   ...
   - `SPIO_STYIO_BIN`
   - managed default compiler at `SPIO_HOME/tools/styio/current/bin/styio`
...
3. Accept local compilers without compatibility validation.
   - Rejected because that would make the managed default compiler path too easy to populate with a binary the current `spio` release cannot actually use.
4. Store managed compilers outside `SPIO_HOME`.

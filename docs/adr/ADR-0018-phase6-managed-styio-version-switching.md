# ADR-0018: Phase-6 Activates Managed `styio` Version Switching Through `pafio tool use`

**Purpose:** Record the decision, context, alternatives, and consequences for switching the active managed `styio` compiler after multiple versions have been installed under `PAFIO_HOME`.
...
`pafio tool install` now installs local compatible `styio` executables into versioned roots under `PAFIO_HOME/tools/styio/<channel>/<compiler-version>/` and refreshes a managed default compiler under `PAFIO_HOME/tools/styio/current/`.
...
2. `tool use` only targets already installed managed compiler roots under `PAFIO_HOME/tools/styio/`.
...
5. Promoting a managed compiler to current refreshes:
   - `PAFIO_HOME/tools/styio/current/bin/styio`
   - `PAFIO_HOME/tools/styio/current/install.json`
...
2. Switch by rewriting `PAFIO_STYIO_BIN`.

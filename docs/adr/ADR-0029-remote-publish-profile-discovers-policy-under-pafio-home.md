# ADR-0029: Remote Publish Profiles Discover Write-Origin Policy Under `PAFIO_HOME`

**Purpose:** Record the decision to let remote publish resolve named write-origin policy profiles under `PAFIO_HOME` instead of repeating explicit policy file paths.
...
`ADR-0028` introduced `pafio publish --registry-policy-file <path>`, which fixed the immediate reuse problem for write-origin gateway headers. It still left one operational gap:
...
1. Add `pafio publish --registry-profile <name>`.
2. A profile resolves to:

```text
PAFIO_HOME/server/registry/publish-profiles/<name>.toml
```
...
7. The profile lives under a server-scoped subtree inside `PAFIO_HOME`, not under the client-side registry cache tree.
...
- server-side config stays under `PAFIO_HOME/server/...`, which keeps it separate from client cache state

...
**Purpose:** Freeze the native JSON engineering standards used for public `spio` HTTP contract packages so clients and backend services can develop independently against the same interface artifacts.

**Last updated:** 2026-04-24

## Contract Model

`spio` API contract work uses repository-native JSON packages under `contracts/`.

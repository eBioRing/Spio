...
```text
./scripts/registry-server-gate.py --registry-root https://registry.example.internal --fetch-trust-descriptor https://platform.example.internal/api/pafio-registry-control/v1/descriptor --pafio-bin ./build-codex/bin/pafio --json
```
...
```text
./scripts/registry-server-gate.py --registry-root https://registry-upload.example.internal --fetch-trust-descriptor https://platform.example.internal/api/pafio-registry-control/v1/descriptor --publish-header 'X-Pafio-Write-Token: dev-token' --pafio-bin ./build-codex/bin/pafio --json
```
...
```text
./scripts/registry-server-gate.py --registry-root https://registry-upload.example.internal --fetch-trust-descriptor https://platform.example.internal/api/pafio-registry-control/v1/descriptor --publish-policy-file /etc/pafio/publish-policy.toml --pafio-bin ./build-codex/bin/pafio --json
```
...
```text
pafio publish --manifest-path path/to/pafio.toml --registry https://registry-upload.example.internal --registry-profile write-dev
```
...
```text
./scripts/registry-server-gate.py --publish-root https://registry-upload.example.internal --fetch-root https://registry.example.internal --fetch-trust-descriptor https://platform.example.internal/api/pafio-registry-control/v1/descriptor --sync-timeout-seconds 30 --pafio-bin ./build-codex/bin/pafio --json
```

Notes:

- `--fetch-trust-descriptor` imports the platform-owned read-root descriptor
  into the gate's isolated `PAFIO_HOME` before fetch validation

# Pafio Registry Deployment Baseline

**Purpose:** Provide the current pragmatic deployment baseline for teams that operate a shared `pafio` registry `v2` root and need safe internal distribution before a hosted publish service is deployed.
...
- pin registry dependencies in `pafio.lock`
...
The executable deployment validation path lives in [../operations/Pafio-Registry-Server-Runbook.md](../operations/Pafio-Registry-Server-Runbook.md).

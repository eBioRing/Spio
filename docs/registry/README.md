...
**Purpose:** Separate local package-manager registry client and offline package rules from server-side package distribution and registry control-plane ownership now moved to `styio-cloud`.
...
- deployment and operational baseline handoff to `styio-cloud`
- industrial `v2` static read-plane protocol and publish-control-plane split

## Ownership

- static read-plane layout and object format remain owned by [Pafio-Registry-V2-Protocol.md](./Pafio-Registry-V2-Protocol.md)
- security-sensitive auth/account/trust boundaries live in [../security/Pafio-Private-Security-Module-Contract.md](../security/Pafio-Private-Security-Module-Contract.md)
- registry HTTP control-plane contract is retained here as a client compatibility reference and is owned operationally by `styio-cloud`
- industrial `v2` read-plane rules live in [Pafio-Registry-V2-Protocol.md](./Pafio-Registry-V2-Protocol.md)
- industrial `v2` publish-plane responsibilities live in [Pafio-Registry-V2-Publish-Control-Plane.md](./Pafio-Registry-V2-Publish-Control-Plane.md)
- client-side consumption rules live in [Pafio-Registry-Client-Contract.md](./Pafio-Registry-Client-Contract.md)
- deployment baseline lives in [Pafio-Registry-Deployment-Baseline.md](./Pafio-Registry-Deployment-Baseline.md)
- executable server validation steps move to `styio-cloud`; this repo keeps package-manager fetch/publish client gates
...
| Capability | `pafio` coverage |
|------------|-----------------------|
| Publish | Local `pafio publish` preflight, filesystem/remote publish command semantics, immutable release expectations, and compatibility with the shared registry-control-plane package. |
...
| Cache | `PAFIO_HOME/registry/` owns cached metadata, blobs, and extracted snapshots; cached artifacts are re-verified before reuse. |

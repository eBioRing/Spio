# Pafio Private Security Module Contract

**Purpose:** Define which security-sensitive responsibilities stay out of the open-source `pafio` tree and how closed-source implementations are injected without changing the public command or repository contracts.
...
- [RegistrySecurity.hpp](<pafio-workspace>/src/PafioSecurity/RegistrySecurity.hpp)
...
Registry consumption uses the public read-side interface in [RegistrySecurity.hpp](<pafio-workspace>/src/PafioSecurity/RegistrySecurity.hpp).
...
Remote publish uses the public write-side interface in [RegistrySecurity.hpp](<pafio-workspace>/src/PafioSecurity/RegistrySecurity.hpp).
...
- [ResolveDefaultRegistryReadSecurity](<pafio-workspace>/src/PafioSecurity/RegistrySecurity.hpp)
- [ResolveDefaultRegistryWriteSecurity](<pafio-workspace>/src/PafioSecurity/RegistrySecurity.hpp)
...
```cpp
static pafio::RegistryWriteSecurityDecision PrivateWriteResolver(
    const pafio::RegistryWriteSecurityRequest &request)
{
  ...
  {
    return pafio::ResolveDefaultRegistryWriteSecurity(request);
  }
  ...
  {
    throw pafio::PublishError("private write policy requires https");
  }

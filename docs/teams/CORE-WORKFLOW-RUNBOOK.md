...
8. Temporary platform compatibility surfaces such as `pafio cloud` until they are consumed from `pafio`
...
8. Keep any remaining local cloud compatibility deterministic and small; new cloud stress or scheduler behavior belongs in `pafio`.
9. Keep `pafio cloud plan --json` targeting the native JSON `pafio` submit-job route, currently `POST /api/pafio/v1/jobs`, and update native tests when that route changes.
...
12. Keep source-build provenance, branch-channel inputs, and cloud-plan submit payloads aligned with the golden-standard submit gate. Managed product branches are `release`, `stable`, and `nightly`; any other source ref is an explicit temporary branch input, not a named staging lane.
...
Record the affected command path, fixture or test coverage gap, the selected project toolchain mode, and whether the next recovery step needs a published external `styio` binary, a source-build checkout, or a downstream `pafio` change.

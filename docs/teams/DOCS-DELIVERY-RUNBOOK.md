...
4. Keep the shared `pafio` / `styio-nightly` toolchain baseline explicit in docs and CI: Debian 13, LLVM 18.1.x, CMake/CTest 3.31.6, and Python 3.13.5.
...
6. Keep repo entry docs and closure docs aligned: `README.md`, `docs/BUILD-AND-DEV-ENV.md`, `docs/planning/Pafio-Master-Plan.md`, `docs/planning/Pafio-Stage-Review-and-Future-Features.md`, `docs/planning/Pafio-Workstreams-and-TODOs.md`, `docs/operations/Pafio-Verification-Matrix.md`, `docs/operations/Pafio-Cloud-Compile-Stress-Runbook.md`, and `docs/operations/Pafio-Repo-Split-Runbook.md` must agree on wrapper-vs-binary entrypoints, current implementation status, and root-relative command paths.
...
10. Keep `docs/planning/Pafio-Styio-Cloud-Migration-Handoff.md` aligned with downstream `styio-cloud` docs when server/platform ownership moves.
...
17. Keep `local-ci-gate` sibling checkouts on the PR target lane when it is `release`, `stable`, or `nightly`, and fall back only to the managed stable/nightly baseline when a temporary branch has no sibling checkout.
...
19. When CI gate grouping, branch-promotion policy, or golden-standard submit readiness changes, update `.github/workflows/local-ci-gate.yml`, `docs/specs/GOLDEN-STANDARD-TEST-SUITE.md`, `docs/specs/POST-COMMIT-CI-CHECKS.md`, generated indexes, and this runbook in the same checkpoint.
...
4. Styio / Contracts reviews `pafio` handoff wording.

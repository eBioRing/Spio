...
class WorkspaceRecord:
    ...
    def refresh_export(self) -> None:
        export_dir = self.pafio_home / "hosted-exports"
...
class HostedWorkspaceServer:
    def __init__(self, *, pafio_bin: pathlib.Path, bind: str, port: int):
        self.pafio_bin = pafio_bin
        self.bind = bind
        self.port = port
    ...
    def open_workspace(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        manifest_path = (
            ...
            if manifest_value
            else (workspace_root / "pafio.toml").resolve()
        )
        ...
        if isinstance(workspace_id, str) and workspace_id in self.records:
            ...
            pafio_home_value = body.get("pafio_home")
            if pafio_home_value:
                pafio_home = pathlib.Path(str(pafio_home_value)).resolve()
                pafio_home.mkdir(parents=True, exist_ok=True)
            else:
                pafio_home = pathlib.Path(
                    tempfile.mkdtemp(prefix=f"pafio-hosted-{workspace_id}-")
                )
    ...
    def tool_install(self, workspace_id: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        return self._run_pafio_command(
            record,
            "tool install",
    ...
    def tool_use(self, workspace_id: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        return self._run_pafio_command(record, "tool use", args)
    ...
    def tool_pin(self, workspace_id: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(channel, str) and channel:
            args.extend(["--channel", channel])
        return self._run_pafio_command(record, "tool pin", args)
    ...
    def fetch_dependencies(self, workspace_id: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        if body.get("offline") is True:
            args.append("--offline")
        return self._run_pafio_command(record, "fetch", args)
    ...
    def vendor_dependencies(self, workspace_id: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        if body.get("offline") is True:
            args.append("--offline")
        return self._run_pafio_command(record, "vendor", args)
    ...
    def run_workflow(
        ...
        response = self._run_pafio_command(
            record,
            command,
            args,
            cwd=working_root,
    ...
    def pack_project(self, workspace_id: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(output_path, str) and output_path:
            args.extend(["--output", output_path])
        return self._run_pafio_command(record, "pack", args)
    ...
    def prepare_publish(self, workspace_id: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(output_path, str) and output_path:
            args.extend(["--output", output_path])
        return self._run_pafio_command(record, "publish", args)
    ...
    def _workspace_env(self, record: WorkspaceRecord) -> dict[str, str]:
        env = dict(os.environ)
        env["PAFIO_HOME"] = str(record.pafio_home)
        return env
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--pafio-bin",
        default=str(DEFAULT_PAFIO),
        help="pafio entrypoint to delegate hosted actions to",
    )
...
def main(argv: list[str]) -> int:
    ...
    if not pafio_bin.exists():
        print(
            json.dumps({"message": f"pafio binary not found: {pafio_bin}"}),
            file=sys.stderr,
        )
        return 2

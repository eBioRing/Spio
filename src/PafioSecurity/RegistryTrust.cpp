...
json LoadTrustStore(const fs::path &pafio_home)
{
...
void WriteTrustStore(const fs::path &pafio_home, const json &payload)
{
  const fs::path store_path = pafio::RegistryTrustStorePath(pafio_home);
  ...
  const fs::path temp_path = store_path.parent_path() / (store_path.filename().string() + ".tmp");
  {
    ...
    if (!out)
    {
      throw pafio::CacheError("failed to open temporary registry trust store: " + temp_path.string());
    }
    out << payload.dump(2) << '\n';
    if (!out.good())
    {
      throw pafio::CacheError("failed to write temporary registry trust store: " + temp_path.string());
    }
  }
  std::error_code ec;
  ...
  {
    fs::remove(temp_path);
    throw pafio::CacheError("failed to finalize registry trust store: " + store_path.string());
  }
}

pafio::RegistryTrustPin PinFromJson(const json &payload)
{
  return {
...
std::vector<RegistryTrustPin> LoadRegistryTrustPins(const fs::path &pafio_home)
{
...
std::optional<RegistryTrustPin> ResolveRegistryTrustPin(const fs::path &pafio_home, const std::string &registry_root)
{
...
RegistryTrustPin ImportRegistryTrustDescriptor(const fs::path &pafio_home, const std::string &descriptor_source)
{

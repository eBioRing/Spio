...
void CopySnapshotTree(const fs::path &source_root, const fs::path &destination_root)
{
  ...
  {
    throw pafio::VendorError("vendored source snapshot does not exist: " + source_root.string());
  }
  if (!fs::is_directory(source_root))
  {
    throw pafio::VendorError("vendored source snapshot is not a directory: " + source_root.string());
  }
  ...
  std::ofstream marker(destination_root / ".pafio-snapshot-ready");
  if (!marker)
  {
    throw pafio::VendorError("failed to write vendor ready marker: " + (destination_root / ".pafio-snapshot-ready").string());
  }
  marker << "ready\n";
}

std::vector<VendoredSnapshot> CollectVendoredSnapshots(const pafio::ResolvedGraphResult &graph)
{
  ...
  for (const pafio::ResolvedPackage &package : graph.packages)
  {
    ...
    {
      throw pafio::VendorError("resolved git package is missing snapshot metadata: " + package.id);
    }
    snapshots.push_back({
        .source = *package.git,
        .rev = *package.rev,
...
VendorResult WriteVendorTree(const VendorRequest &request)
{
  ...
  const fs::path metadata_path = vendor_root / "pafio-vendor.json";
  const json metadata{
      {"tool", "pafio"},

...
bool IsExcludedGeneratedTopLevel(const fs::path &relative_path)
{
  ...
  return first == ".git" || first == ".spio" || first == "dist" || first == "build" || first.starts_with("build-");
}
...
std::string ReadBinaryFile(const fs::path &path)
{
  ...
  if (!in)
  {
    throw spio::PackError("failed to read package source file: " + path.string());
  }
...
PackageSelection SelectPackageForPack(const spio::PackRequest &request)
{
  ...
  if (candidates.empty())
  {
    throw spio::PackError("pack requires a manifest with a local [package] target");
  }
  ...
  if (request.package_name.has_value())
  {
    ...
    if (matches.empty())
    {
      throw spio::PackError("selected package was not found under the active manifest: " + *request.package_name);
    }
    if (matches.size() > 1U)
    {
      throw spio::PackError("selected package is ambiguous under the active manifest: " + *request.package_name);
    }
  ...
  throw spio::PackError("workspace pack is ambiguous; select a root package with --package <namespace/name>");
}
...
std::vector<PackEntry> CollectPackEntries(const PackageSelection &selection, const fs::path &archive_path)
{
  ...
  const fs::path lockfile_path = package_root / "spio.lock";
  ...
  entries.push_back({
      .archive_path = "spio.toml",
      .source_path = std::nullopt,
      .inline_contents = spio::SerializeManifestCanonical(selection.manifest),
  });
  ...
  for (fs::recursive_directory_iterator it(package_root), end; it != end; ++it)
  {
    ...
    {
      throw spio::PackError("pack does not support symlinks inside the package tree: " + absolute_entry.string());
    }
    ...
    {
      throw spio::PackError("pack only supports regular files inside the package tree: " + absolute_entry.string());
    }
...
void WriteOctalField(std::array<char, 512> &header, size_t offset, size_t length, uint64_t value)
{
  ...
  {
    throw spio::PackError("archive field exceeds ustar numeric limit");
  }
...
void WriteChecksumField(std::array<char, 512> &header, uint64_t checksum)
{
  ...
  if (octal.size() > 6U)
  {
    throw spio::PackError("archive checksum exceeds ustar limit");
  }
...
void SplitArchivePathForUstar(const std::string &path, std::string &name, std::string &prefix)
{
  if (path.empty())
  {
    throw spio::PackError("archive entry path must not be empty");
  }
  if (path.size() <= 100U)
  {
    name = path;
  ...
  throw spio::PackError("archive entry exceeds ustar path limits: " + path);
}
...
void WriteTarArchive(const std::vector<PackEntry> &entries, const fs::path &archive_path, std::string_view archive_prefix)
{
  ...
  try
  {
    ...
    if (!out)
    {
      throw spio::PackError("failed to create package archive: " + temp_path.string());
    }
    ...
    if (!out.good())
    {
      throw spio::PackError("failed to finish package archive: " + temp_path.string());
    }
    out.close();
    if (!out)
    {
      throw spio::PackError("failed to flush package archive: " + temp_path.string());
    }

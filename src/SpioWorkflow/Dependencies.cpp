...
{
  if (section == spio::DependencySection::kDevDependencies)
  {
...
const std::vector<spio::Dependency> &SelectSection(const spio::PackageConfig &package, spio::DependencySection section)
{
  if (section == spio::DependencySection::kDevDependencies)
  {
    return package.dev_dependencies;
...
void EnsurePackageManifest(spio::ManifestDocument &manifest, const fs::path &manifest_path, const std::string &command)
{
  if (!manifest.package.has_value())
  {
    throw spio::ValidationError(command + " requires a manifest with [package]: " + manifest_path.string());
  }
}
...
spio::DependencyCommandResult CommitManifestEdit(
    ...
    spio::DependencySection section)
{
  const fs::path lockfile_path = manifest_path.parent_path() / "spio.lock";

  try
  {
    WriteFile(manifest_path, spio::SerializeManifestCanonical(manifest));
    const spio::LockGenerationResult generated = spio::ResolveSingleVersionLockfile(manifest_path);
    WriteFile(generated.lockfile_path, spio::SerializeLockfileCanonical(generated.lockfile));
    return {
...
bool DependencyMatchesTarget(const spio::Dependency &dependency, const std::string &target, bool treat_as_package_name)
{
...
{
  spio::DependencySection section = spio::DependencySection::kDependencies;
  size_t index = 0;
  std::string alias;
...
std::vector<MatchLocation> FindMatches(
    ...
    const std::optional<spio::DependencySection> &requested_section,
    bool treat_as_package_name)
{
  std::vector<MatchLocation> matches;
  auto collect = [&](spio::DependencySection section) {
    const std::vector<spio::Dependency> &dependencies = SelectSection(package, section);
  ...
  collect(spio::DependencySection::kDependencies);
  collect(spio::DependencySection::kDevDependencies);
  return matches;
}
...
DependencyCommandResult AddDependencyAndRefreshLock(const AddDependencyRequest &request)
{
  ...
  const fs::path lockfile_path = request.manifest_path.parent_path() / "spio.lock";

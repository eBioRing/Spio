...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="pafio")
    ...
    check_cmd.add_argument("--manifest-path", default="pafio.toml")
...
def _handle_check(args: argparse.Namespace, as_json: bool) -> int:
    ...
    lock_path = manifest_path.with_name("pafio.lock")
    if lock_path.exists():
        try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.version:
        sys.stdout.write(f"pafio {__version__}\n")
        return EXIT_SUCCESS

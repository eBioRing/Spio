...
def resolve_styio_binary(explicit_path: str | None) -> Path | None:
    candidate = explicit_path or os.getenv("PAFIO_STYIO_BIN")
    if not candidate:
        return None
...
def _find_matching_entry(machine_info: dict, compat_doc: dict) -> dict:
    ...
    raise CompatibilityError(
        "compiler version/channel is outside the published pafio compatibility matrix"
    )
...
def check_compiler_compatibility(binary: Path) -> CompatibilityReport:
    ...
    if expected_compile_plan_versions and not all(
        ...
        raise CompatibilityError(
            "compiler handshake does not provide the compile-plan versions required by this pafio phase"
        )
    ...
    if compiler_edition < supported_edition:
        raise CompatibilityError(
            "compiler edition_max is lower than the minimum edition supported by this pafio phase"
        )
